...
import com.br.jwt.security.LoginDTO;
...
public class LoginController {
	...
	@PostMapping(value={"/api/public/login"}, consumes={"application/json"})
	  public ResponseEntity<?> autentica(@RequestBody LoginDTO loginDTO)
	  {
	    ...
	    RequestEntity<LoginDTO> request = RequestEntity.post(URI.create("http://localhost:9000/api/login")).contentType(MediaType.APPLICATION_JSON).body(loginDTO);
	    try
	    {
